...
static Template transform(const Template& src, double deg, double tx, double ty,
    ...
    for (auto& m : t.minutiae) {
        ...
        m.theta = (float)std::fmod(m.theta + r + 2 * M_PI, 2 * M_PI);
    }
    return t;
}
...
int main() {
    ...
    // --- 각도 정규화 ---
    tu::near(wrap_pi(3 * M_PI), M_PI, 1e-9, "wrap_pi(3pi) == pi");
    tu::near(wrap_pi(-3 * M_PI / 2), M_PI / 2, 1e-9, "wrap_pi(-3pi/2) == pi/2");
    tu::near(std::fabs(wrap_half_pi(M_PI - 0.01)), 0.01, 1e-9,
    ...
    // --- 회전 + 이동 + 잡음 ---
    {
        ...
        tu::near(r.rotation * 180.0 / M_PI, 18.0, 6.0, "추정 회전각 ~= 18도");
    }
